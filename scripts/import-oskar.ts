 ...
 * OSKAR 2026 IMPORT
 *
 * OSKAR2026 sheet'inden ürünleri Supabase'e draft statüsüyle import eder.
 * Header row 8, veri row 9+
 * col[1]=sku, col[2]=name, col[4]=price
 *
 * Flags:
...
function loadRows(): { valid: Row[]; skipped: number } {
    ...
    const ws = wb.Sheets['OSKAR2026'];
    if (!ws) { console.error('[Excel] OSKAR2026 sheet bulunamadı'); process.exit(1); }

    const data: unknown[][] = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' }) as unknown[][];
    const valid: Row[] = [];
    let skipped = 0;

    // Veri row 9+'dan başlıyor (0-based index 9)
    // col[1]=SKU (sipariş kodu), col[2]=name (açıklama), col[4]=fiyat
...
async function main() {
    ...
    console.log(`[Import] OSKAR2026 — ${valid.length + skipped} satır okundu`);
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
        ...
        const payload = chunk.map(row => ({
            ...
            vat_rate:   20,
            currency:   'TRY',
            meta: { source: 'oskar_2026', imported_at: importedAt },
        }));
