...
async function run() {
    try {
        ...
        while (hasMore) {
            console.log(`Fetching Supabase page ${page} (Total processed so far: ${successCount})...`);
            ...
            const { data: products, error } = await supabase
                ...
                .range(0, limitSize - 1); // Get the first 1000 items that STILL need processing
            ...
            if (!products || products.length === 0) {
                console.log('No empty description products found anymore!');
                hasMore = false;
                break;
            }

            const matchedProducts = products.filter(p => p.sku && normalizedExcelSkus.has(p.sku.trim().toUpperCase()));
            console.log(`Of ${products.length} blank descriptions in this chunk, ${matchedProducts.length} are Emes Excel matches.`);

            if (matchedProducts.length === 0) {
                // If there were no matches in these 1000, we must force pagination using range
                hasMore = false;
                console.log("We found 1000 products needing descriptions, but NONE of them were Emes! Skipping logic needs to be complex (fetching all first). Let's fetch ALL blank ones via pagination first.");
                break;
            }
            ...
            for (let i = 0; i < matchedProducts.length; i += chunkSize) {
                ...
                const promises = chunk.map(async (p) => {
                    ...
                    if (updErr) {
                        return { success: false, sku: p.sku, error: updErr.message };
                    }
