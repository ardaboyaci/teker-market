/**
 * FALO MAKARA GÖRSEL SCRAPER
 *
 * DB'deki falo_2026 kaynaklı ürünler için görsel bulur ve bağlar.
 * Ürün adını kullanarak harici site araması yapar.
 *
 * Flags:
...
const OUTPUT_DIR = path.resolve(process.cwd(), 'scripts/output/falo-images');
const LOG_FILE = path.resolve(process.cwd(), 'scripts/output/falo-images-log.json');
...
async function findFaloImage(sku: string, name: string): Promise<string | null> {
    const searchUrls = [
        `https://www.falo.com.tr/arama?q=${encodeURIComponent(sku)}`,
        `https://www.falo.com.tr/arama?q=${encodeURIComponent(name.split(' ').slice(0, 3).join(' '))}`,
        `https://falo.com.tr/search?q=${encodeURIComponent(sku)}`,
    ];
        ...
        try {
            ...
            for (const sel of imgSelectors) {
                ...
                if (src && !src.includes('placeholder') && !src.includes('no-image')) {
                    return src.startsWith('http') ? src : `https://www.falo.com.tr${src}`;
                }
            }
        } catch {
            continue;
...
async function main() {
    console.log('━━━ FALO MAKARA GÖRSEL SCRAPER ━━━');
    ...
    console.log(`[DB] ${dbProducts?.length ?? 0} FALO ürünü görsel bekliyor\n`);
    ...
    for (let i = 0; i < toProcess.length; i++) {
        ...
        const imageUrl = await findFaloImage(product.sku, product.name);
